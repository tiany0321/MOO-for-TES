setwd("C:\\Users\\12758\\Desktop\\GMOO-code\\feature selection  and performance\\latent heat-final\\noise0.07")
library("DiceKriging")

phi=0.07#########noise level prefactor
...
{ set.seed(243)
  ...
  m<-  km(formula<-~.,design=training.data.x, response=training.data.y , noise.var=noise ,covtype = "exp",coef.var=3,optim.method="BFGS",multistart = 1, gr = TRUE)
  set.seed(243)
...
#############x
keep=c("Lambda","VEN","delta.x","ou")
data0=data[,keep]
############y
data.y0=as.data.frame(data[,"enthalpy.lin"])
...
write.csv(heat.train.gauss,"heat.train.gauss.noise0.07.csv")
...
write.csv(data.error.frame,"data.error.frame0.07.csv")
